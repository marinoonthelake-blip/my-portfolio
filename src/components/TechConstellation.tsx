...
const MASTER_DATA = {
  nodes: [
    ...
    // OPS ORBIT
    { id: "Global Ops", group: 4, val: 30, color: "#0070F3", title: "GLOBAL OPS", role: "Scale", desc: "Standardizing operations across APAC and EMEA regions.", bullets: ["Vendor Consolidation", "SOP Standardization", "Operational Excellence"], metrics: ["Cost Reduction", "Reliability", "Global Reach"] },
  ...
  links: [
    ...
    { source: "GenAI", target: "Audio AI" }, { source: "Global Ops", target: "Strategy" }, { source: "Engineering", target: "Analytics" },
...
export default function TechConstellation() {
  ...
  // --- 3. PHYSICS ENGINE (Strict & Instant) ---
  useEffect(() => {
    if (fgRef.current) {
        const graph = fgRef.current;
        
        // Center Point Logic
        const targetX = isMobile ? 0 : 250; // Desktop: Right Side (+250), Mobile: Center (0)
        const targetY = isMobile ? -50 : 0; // Mobile: Slightly Up (-50)
        ...
        // INSTANT START: Point Camera & Zero Zoom
        graph.centerAt(targetX, targetY, 0);
        graph.zoom(0.001, 0); // Single pixel

        // IMMEDIATE EXPLOSION (No Timeout)
        // We use requestAnimationFrame to ensure it happens on the very next paint cycle
        requestAnimationFrame(() => {
           graph.zoom(isMobile ? 2.5 : 3.5, 2000); // 2s Expansion
        });
    }
  }, [isClient, isMobile]);
  ...
  // AUTO-PILOT
  useEffect(() => {
    // Wait 4s after load for user to absorb intro, then start moving
      ...
      <ForceGraph2D
        ...
        nodeCanvasObject={(node, ctx, globalScale) => {
          ...
          for (let i = 0; i <= points; i++) {
              ...
              // Pseudo-random shape based on ID char code
              const dna = (node.id as string).charCodeAt(0) % 5 + 2; 
              const wobble = Math.sin(angle * dna + Date.now() / 1000) * (radius * 0.15);
